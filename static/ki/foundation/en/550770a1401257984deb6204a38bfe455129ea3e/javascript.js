...
continue}if(d&&j.statePlugin){j=j.apply(this)}if(SC.kindOf(j,Ki.State)&&j.isClass&&this[k]!==this.constructor){a=this.createSubstate(j,{name:k,parentState:this,statechart:c});
...
return YES}return NO},enterState:function(){},exitState:function(){},performAsync:function(c,b,a){return Ki.Async.perform(c,b,a)
},toString:function(){return"Ki.State<%@, %@>".fmt(this.get("name"),SC.guidFor(this))
}});Ki.State.plugin=function(b){var a=function(){return SC.objectForPropertyPath(b)
...
this.events=arguments;return this};Ki.StatechartManager={isResponderContext:YES,isStatechart:YES,statechartIsInitialized:NO,rootState:null,monitorIsActive:NO,monitor:null,trace:NO,initMixin:function(){this._gotoStateLocked=NO;
...
}if(!(SC.kindOf(a,Ki.State)&&a.isClass)){throw"Unable to initialize statechart. Root state must be a state class"
}a=this.createRootState(a,{statechart:this,name:Ki.ROOT_STATE_NAME});this.set("rootState",a);
...
for(;b<a;b+=1){d=e[b];switch(d.action){case Ki.EXIT_STATE:c=this._exitState(d.state);
break;case Ki.ENTER_STATE:c=this._enterState(d.state,d.currentState);break}if(SC.kindOf(c,Ki.Async)){this._gotoStateSuspendedPoint={gotoState:f,actions:e,marker:b+1};
...
return undefined}this._sendEventLocked=YES;g=f.get("length");for(;e<g;e+=1){j=NO;
c=f[e];if(!c.get("isCurrentState")){continue}while(!j&&c){if(c.tryToPerform){try{j=c.tryToHandleEvent(a,d,b)
}catch(h){}}if(!j){c=c.get("parentState")}}}this._sendEventLocked=NO;this._flushPendingSentEvents();
return c},_createStateChain:function(b){var a=[];while(b){a.push(b);b=b.get("parentState")
}return a},_findPivotState:function(c,b){if(c.length===0||b.length===0){return null
...
}else{if(f){this._traverseStatesToEnter(f,null,null,g,d)}else{h.currentState=YES}}}}else{if(e.length>0){d.push({action:Ki.ENTER_STATE,state:b});
...
if(!a){return}this.sendEvent(a.event,a.sender,a.context)},_monitorIsActiveDidChange:function(){if(this.get("monitorIsActive")&&SC.none(this.get("monitor"))){this.set("monitor",Ki.StatechartMonitor.create())
}}.observes("monitorIsActive")};Ki.ROOT_STATE_NAME="__ROOT_STATE__";Ki.EXIT_STATE=0;
Ki.ENTER_STATE=1;Ki.Statechart=SC.Object.extend(Ki.StatechartManager);Ki.Async=SC.Object.extend({func:null,arg1:null,arg2:null,tryToPerform:function(d){var c=this.get("func"),b=this.get("arg1"),a=this.get("arg2"),e=SC.typeOf(c);
